...
public class WareSkuServiceImpl extends ServiceImpl<WareSkuMapper, WareSkuEntity> implements WareSkuService {
    ...
    public List<SkuLockVo> checkAndLock(List<SkuLockVo> lockVos, String orderToken) {
        ...
        //判断是否所有商品锁定成功,如果有一个锁定 失败,所有都解锁库存
    ...
    private void checkLock (SkuLockVo lockVo){
        RLock fairLock = this.redissonClient.getFairLock(KEY_PREFIX + lockVo.getSkuId());
        try {
            fairLock.lock();
            //验库存:本质就是查询
            ...
            //锁库存:本质就是更新库存表中的stock-lock字段
            //大数据提供库存接口，获取就近的库存id。这里取第一个库存。
